...
install_proxy_software() {
    ...
    # Install SOCKS5 proxy (dante-server)
    if ! command -v sockd &>/dev/null; then
        log_info "Installing Dante SOCKS5 server..."
        ...
        log_success "Dante SOCKS5 server installed"
    else
        log_success "Dante already installed"
    fi
...
# Block certain destinations (optional security)
socks block {
...
EOF
    ...
    # Create system user for dante if not exists
    if ! id -u dante &>/dev/null; then
        useradd -r -s /bin/false dante
    fi
    
    # Add proxy user to system
