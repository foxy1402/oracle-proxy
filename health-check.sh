...
###############################################################################
# Oracle Cloud Proxy Health Check Script
...
echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   Oracle Cloud Proxy Health Check         ║${NC}"
echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
echo ""
...
if command -v getenforce &>/dev/null; then
    ...
    if [ "$SELINUX_STATUS" = "Permissive" ] || [ "$SELINUX_STATUS" = "Disabled" ]; then
        echo -e "SELinux: ${GREEN}$SELINUX_STATUS (OK)${NC}"
        ((PASS++))
    else
        echo -e "SELinux: ${YELLOW}$SELINUX_STATUS (may block proxy)${NC}"
        ((WARN++))
    fi
else
    echo -e "SELinux: ${GREEN}Not installed${NC}"
    ((PASS++))
fi
...
if [ -f /etc/iptables/rules.v4 ]; then
    ...
    echo -e "iptables rules saved: ${YELLOW}No (may not persist reboot)${NC}"
    ((WARN++))
fi
...
if systemctl is-active --quiet firewalld; then
    ...
    if firewall-cmd --list-ports 2>/dev/null | grep -q "1080/tcp"; then
        echo -e "Port 1080/tcp: ${GREEN}Open${NC}"
        ((PASS++))
    else
    ...
    if firewall-cmd --list-ports 2>/dev/null | grep -q "8888/tcp"; then
        echo -e "Port 8888/tcp: ${GREEN}Open${NC}"
        ((PASS++))
    else
    ...
    echo -e "firewalld: ${YELLOW}Not active (using iptables only)${NC}"
    ((WARN++))
fi
...
if [ $FAIL -eq 0 ] && [ $WARN -eq 0 ]; then
    ...
    echo "1. Configure Oracle Cloud Security List (if not done)"
    echo "2. Test connectivity from your device"
    echo "3. Install dashboard: sudo ./install-dashboard.sh"
    exit 0
